...
.PHONY: stubs
stubs:
	stubgen -o . -p bert_binary_jawiki

.PHONY: build-package
build-package:
	$(eval VERSION := $(shell poetry version -s))
	poetry build
	@tar zxf dist/bert_binary_jawiki-$(VERSION).tar.gz -C ./dist
	@cp dist/bert_binary_jawiki-$(VERSION)/setup.py setup.py
	@black setup.py
	@rm -rf dist
...
.PHONY: uninstall
uninstall:
	pip uninstall -y bert-binary-jawiki
...
ifdef VERSION
	...
	git add pyproject.toml bert_binary_jawiki/__init__.py
